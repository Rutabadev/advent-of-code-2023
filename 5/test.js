...
export class PieceWise {
  ...
  toLaTex() {
    ...
    this.pieces.forEach((piece) => {
      ...
      let lower =
        piece.lower_bound == Number.NEGATIVE_INFINITY
          ? "- \\infty"
          : piece.lower_bound;
      let higher =
        piece.upper_bound == Number.POSITIVE_INFINITY
          ? "\\infty"
          : piece.upper_bound;
  ...
  static fillGaps(pieces) {
    ...
    for (let i = 1; i < _pieces.length; i++) {
      ...
      if (gap > 1) {
        let p = new Piece({
          lower_bound: prev.upper_bound + 1,
          upper_bound: _pieces[i].lower_bound - 1,
        });
        filled.push(p);
      }
    ...
    if (last.upper_bound != Number.POSITIVE_INFINITY) {
      let rightBookmark = new Piece({
        lower_bound: filled[filled.length - 1].upper_bound + 1,
      });
...
export class Piece {
  constructor({ shift, lower_bound, upper_bound }) {
    ...
    this.lower_bound = !isNaN(lower_bound)
      ? lower_bound
      : Number.NEGATIVE_INFINITY;
    this.upper_bound = !isNaN(upper_bound)
      ? upper_bound
