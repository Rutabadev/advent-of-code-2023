...
twoDimensionalInput.forEach((line, y) => {
	...
	while((match = gearRegex.exec(lineString)) !== null) {
		...
		// left
		twoDimensionalInput[y][previousXIndex].match(digitRegex) && adjacentDigits.push({x: previousXIndex, y });
		// right
		twoDimensionalInput[y][nextXIndex].match(digitRegex) && adjacentDigits.push({x: nextXIndex, y });
		// top (multiple)
		y !== 0 && twoDimensionalInput[y - 1].slice(previousXIndex, nextXIndex + 1).forEach((character, i) => {
		...
		// bottom (multiple)
		y !== twoDimensionalInput.length && twoDimensionalInput[y + 1].slice(previousXIndex, nextXIndex + 1).forEach((character, i) => {
			if (character.match(digitRegex)) {
				adjacentDigits.push({
					x: previousXIndex + i,
					y: y + 1,
		...
		const adjacentNumbers = adjacentDigits.reduce((acc, adjacentDigit) => {
			const correspondingNumber = numbersPositions.find(number => 
